...
func (s *Session) NewIOCTLRequest(treeId uint32) IOCTLRequestStruct {
...
func (s *Session) NewFSCTLPIPEWAITRequest(pipename string) FSCTLPIPEWAITRequestStruct {
...
// 连接并绑定命名管道，并拿到管道句柄
func (s *Session) ConnectAndWriteStdInPipes(pipename string) (treeid uint32, pipehandle []byte, err error) {
	...
	IOCTLRequest := s.NewIOCTLRequest(treeId)
	...
	// 连接管道
	FSCTLPIPEWAITRequestIn := s.NewFSCTLPIPEWAITRequest(pipename)
	FSCTLPIPEWAITRequestIn.Timeout = timeout
	IOCTLRequest.Buffer = FSCTLPIPEWAITRequestIn
	s.Debug("Sending Ioctl stdin pipe request ["+pipename+"]", nil)
	buf, err := s.send(IOCTLRequest)
	if err != nil {
		s.Debug("", err)
		return 0, nil, err
	}
	res := NewIOCTLResponse()
	s.Debug("Unmarshalling Ioctl stdin pipe response ["+pipename+"]", nil)
	if err = encoder.Unmarshal(buf, &res); err != nil {
		s.Debug("Raw:\n"+hex.Dump(buf), err)
		return 0, nil, err
	}
	// 创建管道请求
	pipeHander, err := s.CreatePipeRequest(treeId, pipename)
	if err != nil {
		return 0, nil, err
	}
	// 将数据写入管道
	err = s.WritePipeRequest(treeId, []byte("cmd"), pipeHander)
	if err != nil {
		return 0, nil, err
	}
...
// 拿到stdin、out、err句柄
func (s *Session) ConnectAndBindNamedPipes(pipename string) (stdinpipe, stdoutpipe, stderrpipe []byte, err error) {
	...
	// 连接输入管道
	pipeIn := pipename + "_in"
	FSCTLPIPEWAITRequestIn := s.NewFSCTLPIPEWAITRequest(pipeIn)
	FSCTLPIPEWAITRequestIn.Timeout = timeout
	IOCTLRequest.Buffer = FSCTLPIPEWAITRequestIn
	s.Debug("Sending Ioctl stdin pipe request ["+pipeIn+"]", nil)
	buf, err := s.send(IOCTLRequest)
	if err != nil {
		s.Debug("", err)
		//return nil, err
	...
	res := NewIOCTLResponse()
	s.Debug("Unmarshalling Ioctl stdin pipe response ["+pipeIn+"]", nil)
	if err := encoder.Unmarshal(buf, &res); err != nil {
		s.Debug("Raw:\n"+hex.Dump(buf), err)
	}
	stdIn = res.GUIDHandle
	s.Debug("Completed Ioctl stdin pipe ["+pipeIn+"]", nil)
	// 连接输出管道
	pipeOut := pipename + "_out"
	FSCTLPIPEWAITRequestOut := s.NewFSCTLPIPEWAITRequest(pipeOut)
	FSCTLPIPEWAITRequestOut.Timeout = timeout
	IOCTLRequest.Buffer = FSCTLPIPEWAITRequestOut
	s.Debug("Sending Ioctl stdout pipe request ["+pipeOut+"]", nil)
	buf, err = s.send(IOCTLRequest)
	if err != nil {
		s.Debug("", err)
		//return nil, err
	...
	res = NewIOCTLResponse()
	s.Debug("Unmarshalling Ioctl stdout pipe response ["+pipeOut+"]", nil)
	if err = encoder.Unmarshal(buf, &res); err != nil {
		s.Debug("Raw:\n"+hex.Dump(buf), err)
	}
	s.Debug("Completed Ioctl stdout pipe ["+pipeOut+"]", nil)
	// 创建
	// 连接错误管道
	pipeErr := pipename + "_err"
	FSCTLPIPEWAITRequestErr := s.NewFSCTLPIPEWAITRequest(pipeErr)
	FSCTLPIPEWAITRequestErr.Timeout = timeout
	IOCTLRequest.Buffer = FSCTLPIPEWAITRequestErr
	s.Debug("Sending Ioctl stderr pipe request ["+pipeErr+"]", nil)
	buf, err = s.send(IOCTLRequest)
	if err != nil {
		s.Debug("", err)
		//return nil, err
	...
	res = NewIOCTLResponse()
	s.Debug("Unmarshalling Ioctl stderr pipe response ["+pipeErr+"]", nil)
	if err = encoder.Unmarshal(buf, &res); err != nil {
		s.Debug("Raw:\n"+hex.Dump(buf), err)
	}
	s.Debug("Completed Ioctl stderr pipe ["+pipeErr+"]", nil)
