...
// 创建请求结构
type SMB2CreateRequestStruct struct {
	smb.SMB2Header
...
// 创建请求响应结构
type SMB2CreateResponseStruct struct {
	smb.SMB2Header
	StructureSize        uint16
	Oplock               uint8 //1字节，对应文档RequestedOplockLevel字段
	ResponseFlags        uint8
...
// 创建文件请求
func (s *Session) NewSMB2CreateRequest(treeId uint32, filename string, r SMB2CreateRequestStruct) SMB2CreateRequestStruct {
...
// 创建请求响应
func NewSMB2CreateResponse() SMB2CreateResponseStruct {
	smb2Header := NewSMB2Header()
	return SMB2CreateResponseStruct{
...
func (s *Session) SMB2CreateRequest(treeId uint32, filename string, r SMB2CreateRequestStruct) (fileId []byte, err error) {
	s.Debug("Sending Create file request ["+filename+"]", nil)
	req := s.NewSMB2CreateRequest(treeId, filename, r)
	buf, err := s.send(req)
	if err != nil {
		s.Debug("", err)
		return nil, err
	}
	res := NewSMB2CreateResponse()
...
func (s *Session) CreatePipeRequest(treeId uint32, pipename string) (fileId []byte, err error) {
	...
	fileId, err = s.SMB2CreateRequest(treeId, pipename, r)
	if err != nil {
		s.Debug("", err)
		return nil, err
	}
