...
type SMB2ReadRequestStruct struct {
	smb.SMB2Header
...
type SMB2ReadResponseStruct struct {
	smb.SMB2Header
...
type SMB2ReadResponseStruct2 struct {
	smb.SMB2Header
	StructureSize uint16
	DataOffset    uint32 `smb:"fixed:4"`
	Reserved      uint32 `smb:"fixed:4"`
...
func (s *Session) NewSMB2ReadRequest(treeId uint32, fileId []byte) SMB2ReadRequestStruct {
...
func NewReadResponse() SMB2ReadResponseStruct2 {
	return SMB2ReadResponseStruct2{}
}

func (s *Session) ReadRequest(treeId uint32, fileId []byte) (info []byte, err error) {
	s.Debug("Sending Read request", nil)
	req := s.NewSMB2ReadRequest(treeId, fileId)
	req.ReadLength = 1024
	buf, err := s.send(req)
	if err != nil {
		s.Debug("", err)
	...
	res := NewReadResponse()
	s.Debug("Unmarshalling Read response", nil)
	...
	s.Debug("Completed Read response", nil)
	return res.Info, nil
}
